...
| Consumer group | Grupo de consumidores de un stream en el que cada elemento se entrega a un solo miembro y se espera un ACK por mensaje. [T12, p. 32] Ver [Estado distribuido](temas/estado-distribuido.md). |
...
| Inbox transaccional | Tabla de identificadores de mensaje con restricción `UNIQUE` que se inserta en la misma transacción que el cambio de dominio, para detectar reentregas. [T12, p. 21] |
